...
public class FileManager {
    ...
    public static void getScreenshot(String screenshotName) {
        utils.Logs.debug("Tomando Screenshot");
        ...
        try {
            ...
            utils.Logs.error("Error tomar screenshot: %s", ioException.getLocalizedMessage());
        }

    }

    public static void getPageSource(String fileName){
        utils.Logs.debug("Tomando el Page Source de la pagina");

        final var path = String.format("%s/page-source-%s.html", pageStructurePath, fileName);

        try {
            final var file = new File(path);
            utils.Logs.debug("Preguntando si los directorios padres si es que no existe");
            ...
            utils.Logs.error("Error en la toma de page source %s", ioException.getLocalizedMessage());
        }
    }

    public static void deletePreviousEvidence(){
        utils.Logs.debug("Borrando la evidencia Anterior");
        try {
            FileUtils.deleteDirectory(new File(screenshotPath));
            FileUtils.deleteDirectory((new File(pageStructurePath)));
        } catch (IOException ioException) {
            utils.Logs.error("Error al borrar la evidencia anterior  %s",
